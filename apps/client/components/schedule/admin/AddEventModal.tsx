...
const REPEAT_DAY_LABELS = ['M', 'T', 'W', 'T', 'F', 'S', 'S'];

const InlineSpinner = ({ color = '#FFFFFF' }: { color?: string }) => (
  <View
    className="w-7 h-7 rounded-full items-center justify-center"
    style={{ backgroundColor: color === '#FFFFFF' ? 'rgba(255,255,255,0.18)' : 'rgba(29,62,144,0.1)' }}
  >
...
const TimeSpinner = React.memo(({ value, onSelect, range }: { value: string; onSelect: (v: string) => void; range: string[] }) => {
  ...
  return (
    <View className="relative w-20 h-36 rounded-[22px] bg-slate-50 border border-slate-100 overflow-hidden">
      ...
      <FlatList
        ...
        renderItem={({ item }: { item: string }) => {
          ...
          return (
            ...
            >
              <Text style={{ fontSize: isSelected ? 25 : 17, lineHeight: isSelected ? 29 : 21, fontWeight: '900', color: isSelected ? '#1D3E90' : '#CBD5E1' }}>
                {item}
              </Text>
            </TouchableOpacity>
          );
      ...
      />
      <LinearGradient pointerEvents="none" colors={['#F8FAFC', 'rgba(248,250,252,0)']} style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 44 }} />
      <LinearGradient pointerEvents="none" colors={['rgba(248,250,252,0)', '#F8FAFC']} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 44 }} />
    </View>
  );
});
...
export function AddEventModal({
  ...
  const selectedEventTypeLabel = EVENT_TYPE_OPTIONS.find((option) => option.value === newEventType)?.label ?? 'Training';
  const locationOptions = Array.from(
    new Set([newLocation.trim(), ...recentLocations, 'Main Court', 'Sala Polivalenta', 'Training Court'].filter(Boolean))
  );
  const formattedStartDate = newStartDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
  const formattedEndDate = newEndDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
              ...
              <View className="flex-row justify-between items-start gap-4">
                <View className="flex-1">
                  <Text className="text-[12px] font-bold text-slate-400 mb-2">
                    Schedule <Text className="text-slate-300">›</Text> <Text className="text-[#1D3E90]">New {newEventType} event</Text>
                  </Text>
                  <Text className={`${isMobile ? 'text-3xl' : 'text-5xl'} font-black text-[#12309A]`}>Schedule Event</Text>
                  <Text className={`${isMobile ? 'text-sm' : 'text-lg'} text-slate-500 font-medium mt-2`}>
                    Create a training session, game, camp, or internal club event.
                  </Text>
                </View>
                <TouchableOpacity
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: isMobile ? 20 : 40, paddingTop: 24, paddingBottom: 28 }}>
              <View className={`${isMobile ? 'gap-6' : 'flex-row gap-8'}`}>
                <View style={isMobile ? undefined : { width: 360 }} className="gap-5">
                  <LinearGradient
                    colors={['#111C3F', '#1D3E90', '#38BAF8']}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 1 }}
                    style={{ borderRadius: 32, overflow: 'hidden', borderWidth: 4, borderColor: '#FFFFFF', shadowColor: '#1D3E90', shadowOffset: { width: 0, height: 18 }, shadowOpacity: 0.22, shadowRadius: 28, elevation: 10 }}
                  >
                    <View style={{ height: isMobile ? 256 : 420 }} className="relative p-7 justify-between">
                      <View className="flex-row justify-between items-start">
                        ...
                        <View className="bg-white/15 border border-white/20 px-4 py-2 rounded-full">
                          <Text className="text-white font-black text-[10px] uppercase tracking-widest">{newEventType}</Text>
                        </View>
                      </View>
                      ...
                      <View>
                        <Text className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-black text-white mb-3`}>
                          {newTitle.trim() || 'Event Preview'}
                        </Text>
                        <Text className="text-blue-100 font-semibold leading-6">
                          {selectedTeam ? selectedTeam.name : 'Choose a target team'} · {newLocation.trim() || 'Select location'}
                        </Text>
                        {newDescription.trim() ? (
                  ...
                  <View className="bg-white rounded-[28px] p-6 border border-slate-100 shadow-sm">
                    <View className="flex-row items-center mb-4">
                      <Info color="#1D3E90" size={17} />
                      <Text className="text-[#1D3E90] font-black text-base ml-2">Scheduling Tips</Text>
                    </View>
                    <View className="gap-3">
                      <View className="flex-row">
                        <Text className="text-[#38BAF8] font-black mr-3">•</Text>
                        <Text className="flex-1 text-slate-500 font-medium leading-5">Peak court hours are usually late afternoon and early evening.</Text>
                      </View>
                      <View className="flex-row">
                        <Text className="text-[#38BAF8] font-black mr-3">•</Text>
                        <Text className="flex-1 text-slate-500 font-medium leading-5">Recurring trainings create one session per selected weekday.</Text>
                      </View>
                    </View>
                  </View>
                </View>
                ...
                <View className="flex-1 gap-6">
                  <View className="bg-white rounded-[32px] p-6 border border-white shadow-sm">
                    <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">Event Type</Text>
                    <TouchableOpacity
                    ...
                    >
                      <View className="flex-row items-center">
                        <View className="w-9 h-9 rounded-xl bg-blue-50 items-center justify-center mr-3">
                          <Dumbbell size={17} color="#1D3E90" />
                        </View>
                        <View>
                          <Text className="font-black text-slate-800">{selectedEventTypeLabel}</Text>
                          <Text className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">Event category</Text>
                        </View>
                      </View>
                      <ChevronDown size={18} color="#94A3B8" />
                    </TouchableOpacity>

                    <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Event Title *</Text>
                    <TextInput
                      ...
                      placeholder="e.g. U16 Defensive Drills"
                    ...
                    <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mt-5 mb-3">Description *</Text>
                    <TextInput
                      ...
                      placeholder="Add focus, logistics, equipment, or arrival notes."
                      placeholderTextColor="#94A3B8"
                    ...
                    <View className={`${isMobile ? 'gap-5' : 'flex-row gap-5'} mt-5`}>
                      <View className="flex-1">
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Location or Link *</Text>
                        <View className={`bg-white border rounded-2xl px-5 py-3 flex-row items-center shadow-sm ${addEventErrors.location ? 'border-red-300 bg-red-50/40' : 'border-slate-100'}`}>
                          <TextInput
                            ...
                            placeholder="Main Court or https://meet..."
                            placeholderTextColor="#94A3B8"
                            value={newLocation}
                      ...
                      <View className="flex-1">
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Target Team *</Text>
                        <TouchableOpacity
                        ...
                        >
                          <View className="flex-row items-center flex-1">
                            <View className="w-9 h-9 rounded-xl bg-blue-50 items-center justify-center mr-3">
                              <Users size={17} color="#1D3E90" />
                            </View>
                            <View className="flex-1">
                              <Text numberOfLines={1} className={`font-black ${selectedTeam ? 'text-slate-800' : 'text-slate-400'}`}>
                                {selectedTeam ? selectedTeam.name : 'Select team'}
                              </Text>
                              <Text className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{teams.length} available</Text>
                            </View>
                          </View>
                          <ChevronDown size={18} color="#94A3B8" />
                        </TouchableOpacity>
                  ...
                  <View className="bg-white rounded-[32px] p-6 border border-white shadow-sm">
                    <View className="flex-row items-center justify-between mb-7">
                      <Text className="text-xl font-black text-[#1D3E90]">Time & Frequency</Text>
                      {newEventType === 'training' && (
                        ...
                        >
                          <Text className={`text-[11px] font-black mr-2 ${isRecurring ? 'text-[#1D3E90]' : 'text-slate-500'}`}>Recurring</Text>
                    ...
                    <View className={`${isMobile ? 'gap-5' : 'flex-row gap-5'} mb-5`}>
                      <TouchableOpacity onPress={() => setShowDatePicker('start')} className="flex-1">
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Start Date *</Text>
                      ...
                      {newEventType !== 'training' && (
                        <TouchableOpacity onPress={() => setShowDatePicker('end')} className="flex-1">
                          <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">End Date *</Text>
                          <View className="bg-white border border-slate-100 rounded-2xl px-5 py-4 flex-row items-center justify-between shadow-sm">
                    ...
                    <View className={`${isMobile ? 'gap-5' : 'flex-row gap-5'} mb-6`}>
                      <View className="flex-1">
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Start Time *</Text>
                      ...
                      <View className="flex-1">
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">End Time *</Text>
                    ...
                    {newEventType === 'training' && isRecurring && (
                      <View>
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Repeat On</Text>
                        ...
                        <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Repeat For</Text>
                        <View className="flex-row flex-wrap gap-2">
                          {[2, 4, 6, 8, 12].map((w) => (
                            ...
                            >
                              <Text className={`font-black text-[11px] ${recurringWeeks === w ? 'text-white' : 'text-slate-500'}`}>{w} weeks</Text>
                            </TouchableOpacity>
                          ))}
                        </View>
                      </View>
                  ...
                  {newEventType === 'camp' && (
                    <View className="bg-white rounded-[32px] p-6 border border-white shadow-sm">
                      <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3">Registration Fee (€)</Text>
                      <TextInput
              ...
              >
                {addingEvent ? (
                  <View className="flex-row items-center gap-3">
                    <InlineSpinner />
                    <Text className="text-white font-black text-base">Publishing...</Text>
                  </View>
                ) : (
                  <View className="flex-row items-center">
                    <Send color="#FFFFFF" size={18} />
                    <Text className="text-white font-black text-base ml-3">Publish Event</Text>
                  </View>
                )}
              </TouchableOpacity>
            </View>
          ...
          <View className="bg-white rounded-[34px] p-6 shadow-2xl border border-slate-100 w-full" style={{ maxWidth: 560 }} onStartShouldSetResponder={() => true}>
            <View className="flex-row justify-between items-start mb-5">
              <View>
                ...
                <Text className="text-2xl font-black text-[#1E293B]">Select Date</Text>
              </View>
          ...
          <View className="bg-white rounded-[30px] px-5 pt-5 pb-5 shadow-2xl border border-slate-100 w-full" style={{ maxWidth: 360 }} onStartShouldSetResponder={() => true}>
            <View className="flex-row items-center justify-between mb-4">
              <View>
                ...
                <Text className="text-xl font-black text-[#1E293B]">Select Time</Text>
              </View>
            ...
            <View className="items-center">
              <View className="flex-row justify-center items-center">
                <View className="items-center">
                  <Text className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Hour</Text>
                  <TimeSpinner
                ...
                <View className="items-center">
                  <Text className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Minute</Text>
                  <TimeSpinner
            ...
            <View className="flex-row justify-end items-center gap-3 mt-5">
              <TouchableOpacity onPress={() => setShowTimePicker(null)} className="px-5 h-12 items-center justify-center">
                <Text className="text-slate-500 font-black">Cancel</Text>
              </TouchableOpacity>
            ...
            <View className="flex-row items-center justify-between mb-5">
              <View>
                <Text className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">
                  {selectSheet === 'type' ? 'Event category' : selectSheet === 'location' ? 'Location preset' : 'Target team'}
                </Text>
                <Text className="text-2xl font-black text-[#1E293B]">
                  {selectSheet === 'type' ? 'Choose Type' : selectSheet === 'location' ? 'Choose Location' : 'Choose Team'}
                </Text>
              </View>
                  ...
                  {locationOptions.map((location) => {
                    ...
                    return (
                      ...
                      >
                        <View className="flex-row items-center flex-1">
                          <View className={`w-10 h-10 rounded-2xl items-center justify-center mr-3 ${selected ? 'bg-[#1D3E90]' : 'bg-slate-50'}`}>
                            <MapPin size={18} color={selected ? '#FFFFFF' : '#94A3B8'} />
                          </View>
                ...
                <View className="gap-3">
                  {teams.length === 0 ? (
                    <View className="rounded-3xl border border-slate-100 bg-slate-50 p-6 items-center">
                      ...
                      <Text className="font-black text-slate-500 mt-3">No teams loaded</Text>
                    </View>
                  ) : (
                    teams.map((team) => {
                      ...
                      return (
                        ...
                        >
                          <View className="flex-row items-center flex-1">
                            <View className={`w-10 h-10 rounded-2xl items-center justify-center mr-3 ${selected ? 'bg-[#1D3E90]' : 'bg-slate-50'}`}>
                              <Users size={18} color={selected ? '#FFFFFF' : '#94A3B8'} />
                            </View>
