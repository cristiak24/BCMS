...
import { EVENT_TYPE_META, formatEventTime, formatTimeRange, isCancelledEvent } from '../scheduleShared';
            ...
            <View className="flex-row items-start justify-between gap-4">
              <View className="flex-1">
                <View className="flex-row items-center gap-2">
                  ...
                  {isToday && (
                    <View className="bg-[#EAF2FF] rounded-full px-2 py-[2px]">
                      <Text className="text-[9px] font-black uppercase tracking-widest text-[#1D3E90]">Today</Text>
                    </View>
                  )}
                </View>
                ...
                <Text className="text-slate-400 font-bold text-[12px] mt-0.5">
                  {events.length === 0 ? 'Nothing scheduled' : `${events.length} event${events.length === 1 ? '' : 's'} planned`}
                </Text>
              </View>
              <View className="flex-row items-center gap-2">
                <TouchableOpacity
                ...
                >
                  ...
                  <Text className="text-white text-[11px] font-black uppercase tracking-widest">Add</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={onClose}
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20 }}>
            {events.length === 0 ? (
              <View className="rounded-[22px] border border-dashed border-[#D8E4F2] bg-[#F8FAFD] px-6 py-10 items-center">
                ...
                <Text className="text-[#0E2041] font-black mt-4">No events this day</Text>
                <Text className="text-slate-400 font-semibold text-[12.5px] mt-1 text-center">
                  Add a training, match, camp or club event.
                </Text>
                <TouchableOpacity
                  onPress={() => day && onQuickAdd(day.date)}
                  className="mt-5 bg-[#EAF1FB] border border-[#D6E4F7] rounded-full px-5 py-2.5"
                >
                  <Text className="text-[#1D3E90] text-[12px] font-black uppercase tracking-widest">+ Add event</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View>
                      ...
                      <View className="items-center" style={{ width: 18 }}>
                        <View
                          style={{
                            ...
                            borderRadius: 5,
                            marginTop: 15,
                            backgroundColor: cancelled ? '#CBD5E1' : meta.solid,
                          }}
                        />
